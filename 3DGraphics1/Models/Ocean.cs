...
namespace ComputerGraphics3DL1
{
    ...
    public class Ocean
    {
        ...
        private readonly Effect _oceanEffect;

        private readonly List<VertexPositionNormalTexture> _vertices;

        private Texture2D[] OceanNormalMaps;

        public Ocean(GraphicsDevice graphicsDevice, Camera camera, ContentManager contentManager)
        {
            _camera = camera;
            ...
            _oceanEffect = contentManager.Load<Effect>("OceanShader");

            OceanNormalMaps = new Texture2D[4];
            for (var i = 0; i < 4; i++)
                OceanNormalMaps[i] = contentManager.Load<Texture2D>("Ocean" + (i + 1) + "_N");
        }

        public void Draw(GameTime gameTime, TextureCube skyTexture)
        {
            var graphicsDevice = _oceanEffect.GraphicsDevice;
            graphicsDevice.RasterizerState = new RasterizerState { FillMode = FillMode.Solid };
            graphicsDevice.SetVertexBuffer(_vertexBuffer);
            graphicsDevice.Indices = _indexBuffer;
  
            _oceanEffect.Parameters["World"].SetValue(Matrix.Identity);
